...
export const getSilph = async (player) => {
    try {
        ...
        const pokemon = $('#networkAndAchievements > div.arenaHistory.cardBlock > div.content > div.display.bouts > div.tournament')
            .find('.pokemon')
            .toArray()
            .map(element => $(element).attr('title'));

        const sprite = $('#networkAndAchievements > div.arenaHistory.cardBlock > div.content > div.display.bouts > div > div.playerDetails > div.team > div')
            .find('img')
            .toArray()
